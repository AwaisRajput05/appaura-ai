// companyreport.jsx — Company Monthly Report History with Vendor Tracking (Single File)
...
// Detail View Modal (for monthly report details)
const DetailViewModal = ({ isOpen, onClose, data, loading }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`${data?.companyName || 'Company'} - Detailed Report`} size="lg" className="max-h-[90vh] overflow-y-auto">
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div><p className="text-xs text-gray-500">Company</p><p className="text-sm font-medium">{data.companyName}</p></div>
...
<div><p className="text-xs text-gray-500">Period</p><p className="text-sm font-medium">{getMonthName(data.month)} {data.year}</p></div>              <div><p className="text-xs text-gray-500">Payment</p><p className={`text-sm font-medium ${data.paymentStatus === 'PAID' ? 'text-green-600' : 'text-red-600'}`}>{data.paymentStatus}</p></div>
            ...
            {/* Payment Details */}
            <div className="bg-gray-50 p-4 rounded-lg grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div><p className="text-xs text-gray-500">Payment Date</p><p className="text-sm">{data.paymentDate ? formatDisplayDateTime(data.paymentDate) : '-'}</p></div>
              <div><p className="text-xs text-gray-500">Generated At</p><p className="text-sm">{formatDisplayDateTime(data.generatedAt)}</p></div>
            </div>
...
const VendorDetailsModal = ({ isOpen, onClose, vendorId }) => {
  ...
  const fetchVendorDetails = async () => {
    try {
      ...
      const endpoint = apiEndpoints.vendordetails(vendorId);
...
const formatDate = (dateString) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Vendor Details" size="lg" className="max-h-[90vh] overflow-y-auto">
      <div className="p-4 sm:p-6">
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {/* Basic Information */}
            <div>
              <h3 className="text-lg font-semibold text-gray-800 mb-3 pb-2 border-b">Basic Information</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <InfoItem label="Vendor ID" value={vendorData.id} />
                <InfoItem label="Business Name" value={vendorData.businessName} />
                <InfoItem label="Business Type" value={vendorData.businessType} />
                <InfoItem label="Organization Type" value={vendorData.organizationType} />
                <InfoItem label="Status" value={vendorData.status} isStatus />
                <InfoItem label="Subscription Status" value={vendorData.subscriptionStatus} isStatus />
                <InfoItem label="CRN" value={vendorData.crn} />
                <InfoItem label="Username" value={vendorData.username} />
                <InfoItem label="Role" value={vendorData.role} />
              </div>
              {vendorData.businessDescription && (
                <div className="mt-3">
                  <p className="text-xs text-gray-500">Business Description</p>
                  <p className="text-sm bg-gray-50 p-2 rounded">{vendorData.businessDescription}</p>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <InfoItem label="First Name" value={vendorData.firstName} />
                <InfoItem label="Last Name" value={vendorData.lastName} />
                <InfoItem label="Email" value={vendorData.emailAddress} />
                <InfoItem label="Phone" value={vendorData.phoneNumber} />
                <InfoItem label="Business Address" value={vendorData.businessAddress} className="col-span-2" />
                <InfoItem label="City" value={vendorData.city} />
                <InfoItem label="State" value={vendorData.state} />
                <InfoItem label="Zip" value={vendorData.zip} />
                <InfoItem label="Country" value={vendorData.country} />
              </div>
            </div>

            {/* License Information */}
            <div>
              <h3 className="text-lg font-semibold text-gray-800 mb-3 pb-2 border-b">License Information</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <InfoItem label="License Number" value={vendorData.licenseNumber} />
                <InfoItem label="License Expiry" value={vendorData.licenseExpiryDate ? formatDate(vendorData.licenseExpiryDate) : 'N/A'} />
              </div>
            </div>

            {/* Bank Information */}
            <div>
              <h3 className="text-lg font-semibold text-gray-800 mb-3 pb-2 border-b">Bank Information</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <InfoItem label="Bank Name" value={vendorData.bankName} />
                <InfoItem label="Account Number" value={vendorData.accountNumber} />
                <InfoItem label="Branch Code" value={vendorData.branchCode} />
              </div>
            </div>

            {/* System Information */}
            <div>
              <h3 className="text-lg font-semibold text-gray-800 mb-3 pb-2 border-b">System Information</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <InfoItem label="Branch ID" value={vendorData.branchId} />
                <InfoItem label="Is Master" value={vendorData.isMaster ? 'Yes' : 'No'} />
                <InfoItem label="Sub Account Code" value={vendorData.subAccountCode} />
                <InfoItem label="Sub Account Type" value={vendorData.subAccountType} />
                <InfoItem label="Created At" value={vendorData.createdAt ? formatDate(vendorData.createdAt) : 'N/A'} />
                <InfoItem label="Updated At" value={vendorData.updatedAt ? formatDate(vendorData.updatedAt) : 'N/A'} />
              </div>
            </div>
            ...
            {vendorData.applicationRoles && vendorData.applicationRoles.length > 0 && (
              <div>
                <h3 className="text-lg font-semibold text-gray-800 mb-3 pb-2 border-b">Application Roles</h3>
...
export default function CompanyHistory() {
  ...
  // Fetch monthly report history (detail=false for list view)
  const fetchReports = useCallback(async () => {
    ...
    try {
      ...
      const endpoint = apiEndpoints.monthlyReportHistory(
        searchTerm,
        fromDate,
        toDate,
        sort,
        false, // detail=false for list view
        companyStatus,
        paymentStatus,
      ...
      // Map data (detail=false response)
      ...
      setError(getErrorMessage(err) || 'Failed to fetch monthly reports');
      setReportsData([]);
      setReportsPagination((p) => ({ ...p, total: 0 }));
    } finally {
      setLoadingReports(false);
    }
  }, [searchTerm, fromDate, toDate, sort, companyStatus, paymentStatus, reportsPagination.page, reportsPagination.page_size]);
  ...
  // Handle view details (monthly report) - FIXED to use dedicated detail endpoint
  const handleViewDetails = useCallback(async (row) => {
    ...
    try {
      ...
      // Use the dedicated detail endpoint with the report ID
      const endpoint = apiEndpoints.monthlyReportDetail(report.id); // Assumes this exists

      const { data } = await apiService.get(endpoint, { headers });
      
      // The API likely returns the detailed report object directly
  ...
  // Determine if a report is from current month (to disable pay button)
  ...
  }, [view, searchTerm, fromDate, toDate, sort, companyStatus, paymentStatus, reportsPagination.page, reportsPagination.page_size, fetchReports]);
  ...
  inputClassName: "w-full sm:w-36",
},
     {
      type: "search",
      name: "search",
      label: "Search",
      placeholder: "Search by company name or CRN...",
      value: searchTerm,
  ...
  ], [searchTerm, fromDate, toDate, companyStatus, paymentStatus, sort]);
  ...
  cell: ({ row }) => <div className="text-xs sm:text-sm">{getMonthName(row.original.month)}</div>,
},
    {
      ...
      header: "Total Users",
      cell: ({ row }) => <div className="text-xs sm:text-sm text-right">{row.original.totalUsers}</div>,
    },
    {
      accessorKey: "totalEligibleUser",
      header: "Eligible",
      cell: ({ row }) => <div className="text-xs sm:text-sm text-right">{row.original.totalEligibleUser}</div>,
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="text-xs sm:text-sm text-right font-medium text-green-600">
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const report = row.original;
        const currentMonth = isCurrentMonth(report.year, report.month);
        const isPaid = report.paymentStatus === 'PAID';
        const isUnpaid = report.paymentStatus === 'UNPAID';

        return (
          <div className="flex justify-center gap-2">
            <ButtonTooltip tooltipText="View Details" position="top">
              <Button variant="primary" size="sm" onClick={() => handleViewDetails(row)} className="!p-1.5 !min-w-0">
                <Eye className="h-3 w-3 sm:h-4 sm:w-4" />
              </Button>
            </ButtonTooltip>

            <ButtonTooltip tooltipText="View Vendors" position="top">
              <Button variant="info" size="sm" onClick={() => handleViewVendors(row)} className="!p-1.5 !min-w-0">
                <Users className="h-3 w-3 sm:h-4 sm:w-4" />
              </Button>
            </ButtonTooltip>

            {isUnpaid && (
              <ButtonTooltip tooltipText={currentMonth ? "Cannot pay for current month" : "Mark Paid"} position="top">
                <Button
                  variant="success"
                  size="sm"
                  onClick={() => handlePayClick(row)}
                  disabled={currentMonth}
                  className={`!p-1.5 !min-w-0 ${currentMonth ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 sm:p-6 border-b gap-4">
          <div className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-3">
            <span className="text-xl sm:text-2xl font-bold text-gray-800">
              {view === 'reports' ? 'Partner History' : 'Vendor Tracking'}
            </span>
      ...
      {view === 'reports' && (
        <HomeTable
          ...
          onFilterChange={(name, value) => {
            ...
            else if (name === "companyStatus") setCompanyStatus(value);
          ...
          hideDefaultActions
          noDataMessage="No monthly reports found"
        />
      )}
