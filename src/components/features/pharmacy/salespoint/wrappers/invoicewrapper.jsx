...
import { ShoppingBag, ClipboardList, RotateCcw, ChevronDown, Lock } from "lucide-react";
import ReturnMedicineList from "../returninvoice";
...
const ALL_OPTIONS = [
  { id: "all-invoices",    permissionName: "All Invoices",    label: "All Invoices",    icon: ClipboardList, description: "View all sales invoices", color: "text-blue-600" },
  { id: "return-invoices", permissionName: "Return Invoices", label: "Return Invoices", icon: RotateCcw,     description: "View return history",      color: "text-purple-600" },
];
...
export default function InvoicesWrapper() {
  ...
  const [invoicesType, setInvoicesType] = useState(() => visibleOptions[0]?.id ?? "");
  ...
  const renderContent = () => {
    switch (invoicesType) {
      case "all-invoices":    return <AllInvoices />;
      case "return-invoices": return <ReturnMedicineList />;
      default:                return null;
    }
  };
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs sm:text-sm text-gray-500 truncate">View sales or return invoices</p>
              </div>
            </div>
            {visibleOptions.length > 1 && (
              <div className="w-full sm:w-auto flex justify-end">
                <Dropdown selected={invoicesType} onChange={setInvoicesType} options={visibleOptions} />
              </div>
            )}
          </div>
        </div>
