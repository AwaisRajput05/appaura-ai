// DrugStock.jsx — REFACTORED WITH INVENTORY CONSTANTS AND DELETE FUNCTIONALITY
...
export default function MedicineStock() {
  ...
  // Base endpoint without any query params
...
}, [searchTerm, expiryDate, dateFilters.fromDate, dateFilters.toDate, pagination.page, pagination.page_size, selectedBranchesPayload, sortBy]); // Add sortBy here
...
const openEditModal = (drug) => {
  ...
  let rawExpiryDate = drug.raw_expiry_date;
  if (!rawExpiryDate && drug.expiry_date) {
    const parts = drug.expiry_date.split('/');
    if (parts.length === 3) {
      rawExpiryDate = `${parts[2]}-${parts[1]}-${parts[0]}`;
    } else {
      rawExpiryDate = drug.expiry_date;
    }
  }
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      // Date formatting - handle various input formats properly
      ...
      // Include packing data in payload
      const payload = {
        medicine_id: editingDrug.medicine_id.toString(),
        ...
        expiry_date: formattedExpiryDate, // Now properly formatted as YYYY-MM-DD
        
        // Include complete packing information
      ...
      const url = `${apiEndpoints.editMedicine()}?medicine_id=${editingDrug.medicine_id}&batch_code=${encodeURIComponent(editingDrug.batch_code)}&drug_name=${encodeURIComponent(editingDrug.drug_name)}`;

      // USE CONSTANTS FOR AUTH HEADERS
      const headers = INVENTORY_MODULE_CONSTANTS.getAuthHeaders(token, currentVendorId);
      headers["X-User-Branch-Id"] = branchId;

      await apiService.put(url, payload, { headers });
      ...
      // USE CONSTANTS FOR ERROR HANDLING
      setModalError(INVENTORY_MODULE_CONSTANTS.getErrorMessage(err) || "Failed to update medicine");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Construct the delete URL with query parameters
      // type is always "medicine" as requested
  ...
  // Filter fields with date range first
  const filterFields = useMemo(() => [
    // Date range filter in single line - placed first
    {
      type: "dateRange",
      label: "Date Range",
      fromName: "fromDate",
      toName: "toDate",
      value: { fromDate: dateFilters.fromDate, toDate: dateFilters.toDate },
  ...
  ], [searchTerm, expiryDate, dateFilters.fromDate, dateFilters.toDate, handleFilterChange, today]);

  // Export also uses date range params
  const exportParams = useMemo(() => ({
    drug_name: searchTerm.trim() || undefined,
    expiry_date: expiryDate || undefined,
    page: 1,
    page_size: 1000,
    ...(dateFilters.fromDate && { fromDate: dateFilters.fromDate }),
    ...(dateFilters.toDate && { toDate: dateFilters.toDate })
  }), [searchTerm, expiryDate, dateFilters.fromDate, dateFilters.toDate]);
        ...
        <HomeTable
          title={
            <div className="flex items-center gap-3">
              ...
              <span className="text-2xl font-bold text-gray-800">Medicine Stock</span>
            </div>
          }
          data={drugs}
          columns={columns}
  ...
  label="Expiry Date"
  type="date"
  value={editingDrug.rawExpiryDate || ""}
  disabled
  className="locked-input"
/>
        ...
        {deletingDrug && (
          <div className="py-4">
            ...
            <div className="bg-gray-50 p-4 rounded-lg mb-4">
              <p className="text-sm">
                <span className="font-semibold">Medicine:</span> {deletingDrug.drug_name}
              </p>
              <p className="text-sm">
                <span className="font-semibold">Batch Code:</span> {deletingDrug.batch_code}
              </p>
              <p className="text-sm">
                <span className="font-semibold">Manufacturer:</span> {deletingDrug.manufacturer}
              </p>
              <p className="text-sm">
                <span className="font-semibold">Current Stock:</span> {deletingDrug.stock}
              </p>
            </div>
