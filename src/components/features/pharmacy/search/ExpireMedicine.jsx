// ExpireMedicine.jsx - REFACTORED WITH SEARCH_MODULE_CONSTANTS
...
export default function ExpireMedicineTable() {
  // Refs for cleanup
...
const dynamicTitle = useMemo(() => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const trimmed = filters.drug_name.trim();
      const endpoint = apiEndpoints.filterExpiredDrug(pagination.page, pagination.page_size, trimmed);
      ...
      const total = data?.pagination?.total_items || rawData.length;

    const mapped = rawData.map((drug, i) => ({
  id: `${drug.drug_name}-${i}`,
  drug_name: drug.name || "N/A",
  expiry_date: SEARCH_MODULE_CONSTANTS.formatDate(drug.expiry_date, false), 
  status: drug.status || "Expired",
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => <div className="text-sm font-semibold text-red-600">{row.original.expiry_date}</div>,
    },
    {
      accessorKey: "stock",
      header: ({ column }) => <HeaderWithSort column={column} title="Stock" />,
      cell: ({ row }) => <div className="text-sm font-medium text-gray-700">{row.original.stock}</div>,
    },
    {
      accessorKey: "status",
      header: ({ column }) => <HeaderWithSort column={column} title="Status" />,
      cell: ({ row }) => (
        <div className="text-sm font-semibold text-red-700 bg-red-100 px-2 py-1 rounded-full inline-block">
          {row.original.status}
        </div>
      ),
    },
  ...
  ], [filters.drug_name, selectedValue, fullBranchOptions, isMaster]);

  // Export endpoint
  const exportEndpoint = useMemo(() => {
    if (!currentVendorId && !selectedChildVendorId) return null;
    const trimmed = filters.drug_name.trim();
    return apiEndpoints.filterExpiredDrug(1, 1000, trimmed);
  }, [filters.drug_name, currentVendorId, selectedChildVendorId]);
    ...
    />
      ...
      <div>
        ...
        <HomeTable
          ...
          addButtonName="NEW DRUG"
          addButtonPath="/admin-vendors/pharmacy-management/search/expire-medicine/new"
