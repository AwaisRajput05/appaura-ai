// Manufacturer.jsx - REFACTORED WITH SEARCH_MODULE_CONSTANTS
...
const dynamicTitle = useMemo(() => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const manufacturer = searchManufacturer.trim();
      const endpoint = apiEndpoints.searchByManufacturer(manufacturer, pagination.page, pagination.page_size);
  ...
  }, [searchManufacturer, pagination.page, pagination.page_size, selectedBranchId, selectedChildVendorId]);
  ...
  const filterFields = useMemo(() => [
    {
      ...
      name: "manufacturer",
      label: "Manufacturer",
      placeholder: "e.g. GSK, Pfizer, Getz, Searle...",
  ...
  ], [searchManufacturer, selectedValue, fullBranchOptions, isMaster]);

  // Export endpoint
  const exportEndpoint = useMemo(() => {
    return apiEndpoints.searchByManufacturer(searchManufacturer.trim(), 1, 1000);
  }, [searchManufacturer]);
  ...
  <ExportReports
    ...
    data={drugs}
    reportType="Find Medicine by Manufacturer"
  ...
  />
      ...
      <HomeTable
        ...
        addButtonPath="/admin-vendors/pharmacy-management/search/manufacturer/new"
        ...
        error={error}
        onRetry={handleRetry}
        noDataMessage={drugs.length === 0 && !loading && !error ? "Search a manufacturer to see results" : error}
      />
    </div>
  );
}