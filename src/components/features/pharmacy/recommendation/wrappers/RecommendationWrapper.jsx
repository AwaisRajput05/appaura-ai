...
import { Shield, ChevronDown, ThumbsUp, HeartPulse, Lock } from "lucide-react";
import DrugSafetyCheck from '../DrugSafetyCheck';
import DrugRecommend from '../drugrecommend';
...
const ALL_OPTIONS = [
  { id: "safety-check", permissionName: "Safety Check",            label: "Safety Check",            icon: Shield,   description: "Check medicine safety by age",   color: "text-blue-600" },
  { id: "recommend",    permissionName: "Medicine Recommendation", label: "Medicine Recommendation", icon: ThumbsUp, description: "Find alternative medicines",      color: "text-green-600" },
];
...
export default function RecommendationWrapper() {
  ...
  const [recommendationType, setRecommendationType] = useState(() => visibleOptions[0]?.id ?? "");
  ...
  const renderContent = () => {
    switch (recommendationType) {
      case "safety-check": return <DrugSafetyCheck />;
      case "recommend":    return <DrugRecommend />;
      default:             return <DrugSafetyCheck />;
    }
  };
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <div className="p-2 bg-blue-100 rounded-lg flex-shrink-0">
                <HeartPulse className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600" />
              </div>
              <div className="min-w-0">
                <h1 className="text-lg sm:text-xl font-bold text-gray-900 truncate">Medicine Recommendation</h1>
                <p className="text-xs sm:text-sm text-gray-500 truncate">Check safety and find alternative medicines</p>
              </div>
            </div>
            {visibleOptions.length > 1 && (
              <div className="w-full sm:w-auto flex justify-end">
                <Dropdown selected={recommendationType} onChange={setRecommendationType} options={visibleOptions} />
              </div>
            )}
          </div>
        </div>
