...
import { CreditCard, ChevronDown, History, Wallet, Lock } from "lucide-react";
...
const ALL_OPTIONS = [
  { id: "payment-history", permissionName: "Payment History", label: "Payment History", icon: History,    description: "View your payment records",          color: "text-blue-600" },
  { id: "payment-request", permissionName: "Payment Request", label: "Payment Request", icon: CreditCard, description: "Choose a plan and submit payment",   color: "text-green-600" },
];
...
export default function PaymentWrapper() {
  ...
  const [paymentType, setPaymentType] = useState(() => visibleOptions[0]?.id ?? "");
  ...
  const renderContent = () => {
    switch (paymentType) {
      case "payment-history": return <Payhistory />;
      case "payment-request": return <Payment />;
      default:                return <Payhistory />;
    }
  };
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0">
                <h1 className="text-lg sm:text-xl font-bold text-gray-900 truncate">Payment Management</h1>
                <p className="text-xs sm:text-sm text-gray-500 truncate">View payment history and make new requests</p>
              </div>
            </div>
            {visibleOptions.length > 1 && (
              <div className="w-full sm:w-auto flex justify-end">
                <Dropdown selected={paymentType} onChange={setPaymentType} options={visibleOptions} />
              </div>
            )}
          </div>
        </div>
