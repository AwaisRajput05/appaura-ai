// nearexpiryreminder.jsx - REFACTORED WITH CONSTANTS & RESPONSIVE
...
import { Calendar, Clock, History, AlertCircle, RefreshCw, Bell, X } from 'lucide-react';
...
export default function NearExpirySchedule() {
  const { user } = useAuth();
  ...
  // Constants
  const CATEGORY = ENUMS.SCHEDULE_CATEGORY.NEAR_EXPIRY_MEDICINE;
    ...
    try {
      ...
      if (response.data.status === "OK" && response.data.data) {
        ...
        const newSchedule = {
          ...
          nearExpiryMonths: scheduleData.nearExpiryMonths || DEFAULT_VALUES.SCHEDULE.nearExpiryMonths
        };
  ...
  const handleNearExpiryChange = useCallback((e) => {
    const value = e.target.value;
    handleScheduleChange('nearExpiryMonths', value === '' ? null : Number(value));
  }, [handleScheduleChange]);
  
  const handleNearExpiryBlur = useCallback(() => {
    const months = schedule.nearExpiryMonths;
    if (months == null || isNaN(months) || months < ENUMS.LIMITS.NEAR_EXPIRY_MIN) {
      handleScheduleChange('nearExpiryMonths', ENUMS.LIMITS.NEAR_EXPIRY_MIN);
    } else if (months > ENUMS.LIMITS.NEAR_EXPIRY_MAX) {
      handleScheduleChange('nearExpiryMonths', ENUMS.LIMITS.NEAR_EXPIRY_MAX);
    }
  }, [schedule.nearExpiryMonths, handleScheduleChange]);
  ...
  const handleSave = useCallback(async () => {
    try {
      ...
      // Validation
      if (!validateNearExpiryMonths(schedule.nearExpiryMonths)) {
        setError(ERROR_MESSAGES.INVALID_EXPIRY_MONTHS);
        return;
      }
      ...
      const body = {
        ...
        nearExpiryMonths: schedule.nearExpiryMonths,
      };
  ...
  const historyColumns = useMemo(() => [
    ...
    {
      accessorKey: "nearExpiryMonths",
      header: ({ column }) => <HeaderWithSort column={column} title="Months" />,
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          <span className="font-medium text-sm sm:text-base">{row.original.nearExpiryMonths}</span>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 py-4 sm:py-6 px-3 sm:px-4 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-[#30426B] via-[#3C5690] to-[#5A75C7] rounded-xl sm:rounded-2xl shadow-xl p-4 sm:p-6 mb-4 sm:mb-8 text-center text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-center gap-2 sm:gap-4 mb-2 sm:mb-3">
              ...
              <h1 className="text-xl sm:text-2xl md:text-3xl font-bold">Near Expiry Schedule</h1>
            </div>
            <p className="text-blue-100 text-xs sm:text-sm md:text-base font-medium px-2">Configure automated reports for near expiry medicines</p>
          </div>
        </div>
        ...
        {loading && (
          <div className="flex justify-center items-center mb-4 sm:mb-6">
            <div className="flex items-center gap-2 sm:gap-3 bg-white p-3 sm:p-4 rounded-lg sm:rounded-xl shadow-sm">
              <div className="w-4 h-4 sm:w-6 sm:h-6 animate-spin rounded-full border-b-2 border-blue-600"></div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-0 mb-4 sm:mb-6">
              ...
              <div className="flex items-center gap-2 sm:gap-3 self-end sm:self-auto">
                <Button
                  ...
                  variant="icon"
                  size="sm"
                  className="text-gray-600 hover:text-blue-600 p-1.5 sm:p-2"
                >
              ...
              <div className={`border rounded-lg sm:rounded-xl p-3 sm:p-5 ${statusInfo.bgColor}`}>
                <div className="flex items-center justify-between mb-3 sm:mb-4">
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-gray-800 text-sm sm:text-base">Near Expiry</h4>
                ...
                <div className="space-y-2 sm:space-y-3 text-xs sm:text-sm">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600">Near Expiry:</span>
                    <span className="font-medium">{savedSchedule.nearExpiryMonths} months</span>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1 text-xs sm:text-sm">
                      ...
                      <span className={`font-medium ${nextRunDate ? 'text-blue-600' : 'text-gray-500'}`}>
                        {formatDateForDisplay(nextRunDate)}
                      </span>
                    </div>
                    <div className="h-1 bg-gray-200 rounded-full overflow-hidden">
                      <div 
                        className={`h-full ${nextRunDate ? 'bg-blue-500' : 'bg-gray-300'}`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none appearance-none bg-white"
                >
              ...
              <InputText
                ...
                name="nearExpiryMonths"
                type="number"
                value={schedule.nearExpiryMonths ?? ''}
                onChange={handleNearExpiryChange}
                onBlur={handleNearExpiryBlur}
                required={true}
                placeholder={`e.g. ${DEFAULT_VALUES.SCHEDULE.nearExpiryMonths}`}
                maxLength={2}
                className="mb-0"
                labelClassName="text-xs sm:text-sm"
                inputClassName="text-xs sm:text-sm px-3 sm:px-4 py-2 sm:py-3"
                error={schedule.nearExpiryMonths && !validateNearExpiryMonths(schedule.nearExpiryMonths) ? 
                  { message: ERROR_MESSAGES.INVALID_EXPIRY_MONTHS } : null}
              />
      ...
      <Modal
        ...
        title="Near Expiry Schedule History"
        size="xl"
      >
        <div className="p-2 sm:p-4">
          ...
          <div className="mb-4 sm:mb-6">
            <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
              ...
              <div>
                <h3 className="font-bold text-gray-800 text-sm sm:text-base">History: Near Expiry Medicine</h3>
