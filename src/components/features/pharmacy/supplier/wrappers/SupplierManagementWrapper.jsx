...
import { Truck, ChevronDown, ShoppingCart, FileText, Users, Lock } from "lucide-react";
import CreateSupplier from '../createsupplier';
import SupplierOrder from '../supplierorder';
import AllSupplierOrders from '../allsuppliersorders';
...
const ALL_OPTIONS = [
  { id: "suppliers",  permissionName: "Suppliers",       label: "Suppliers",       icon: Users,        description: "Manage supplier details",        color: "text-blue-600" },
  { id: "orders",     permissionName: "Supplier Orders", label: "Supplier Orders", icon: ShoppingCart, description: "Manage orders by supplier",       color: "text-green-600" },
  { id: "all-orders", permissionName: "All Orders",      label: "All Orders",      icon: FileText,     description: "View all supplier orders",       color: "text-purple-600" },
];
...
export default function SupplierManagementWrapper() {
  const visibleOptions = useMemo(() => ALL_OPTIONS.filter((o) => hasTabPermission(o.permissionName)), []);
  const [supplierType, setSupplierType] = useState(() => visibleOptions[0]?.id ?? "");
  ...
  const renderContent = () => {
    switch (supplierType) {
      case "suppliers":  return <CreateSupplier />;
      case "orders":     return <SupplierOrder />;
      case "all-orders": return <AllSupplierOrders />;
      default:           return <CreateSupplier />;
    }
  };
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0">
                <h1 className="text-lg sm:text-xl font-bold text-gray-900 truncate">Supplier Management</h1>
                <p className="text-xs sm:text-sm text-gray-500 truncate">Manage suppliers, orders, and inventory</p>
              </div>
            </div>
            {visibleOptions.length > 1 && (
              <div className="w-full sm:w-auto flex justify-end">
                <Dropdown selected={supplierType} onChange={setSupplierType} options={visibleOptions} />
              </div>
            )}
          </div>
        </div>
