// BranchManagementWrapper.jsx — UNIFIED BRANCH MANAGEMENT (Add Branch & Permissions)
...
import BranchPermission from '../BranchPermisssion';

// ─── Permission helper (same pattern as other wrappers) ─────────────────────
...
// ─── All options with permissionName matching localStorage labels ────────────
const ALL_OPTIONS = [
  { 
    id: "branch-permission", 
    permissionName: "Branch Permissions",
    label: "Branch Permissions", 
    icon: Shield,
    description: "Manage branch permissions",
    color: "text-blue-600"
  },
  { 
    id: "add-branch", 
          ...
          {options.map((opt) => {
            ...
            return (
              <button
                ...
                className={`flex items-start gap-3 w-full px-4 py-3 text-left hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0 ${
                  selected === opt.id ? "bg-blue-50" : ""
                }`}
              >
                <Icon className={`w-5 h-5 ${opt.color} mt-0.5 flex-shrink-0`} />
                <div className="min-w-0 flex-1">
                  <div className={`font-medium truncate ${selected === opt.id ? "text-blue-700" : "text-gray-700"}`}>
                    {opt.label}
                  </div>
...
export default function BranchManagementWrapper() {
  const visibleOptions = useMemo(
    () => ALL_OPTIONS.filter((o) => hasTabPermission(o.permissionName)),
    []
  );
  
  const [branchType, setBranchType] = useState(() => visibleOptions[0]?.id ?? "");
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0">
                <h1 className="text-lg sm:text-xl font-bold text-gray-900 truncate">Branch Management</h1>
                <p className="text-xs sm:text-sm text-gray-500 truncate">Add branches and manage permissions</p>
              </div>
            </div>
            {visibleOptions.length > 1 && (
              <div className="w-full sm:w-auto flex justify-end">
                <Dropdown selected={branchType} onChange={setBranchType} options={visibleOptions} />
              </div>
            )}
          </div>
        </div>
