// UserManagementWrapper.jsx — UNIFIED USER MANAGEMENT (Add User & Active Subaccounts)
...
import ActiveSubaccounts from '../ActiveSubAcc';

const UserManagementDropdown = ({ selectedType, onTypeChange }) => {
  ...
  const options = [
    { 
      ...
      label: 'Add User', 
      icon: UserPlus,
      description: 'Generate sub-account code',
      color: 'text-purple-600'
    },
    { 
      id: 'active-subaccounts', 
      label: 'Active Subaccounts', 
      icon: UserCheck,
      description: 'View active sub-accounts',
  ...
  return (
    <div className="relative w-full sm:w-auto" style={{ zIndex: 45 }}>
      <button
...
export default function UserManagementWrapper() {
  const [userType, setUserType] = useState('add-user');

  const renderContent = () => {
    switch (userType) {
      case 'add-user':
        return <AddUser />;
      case 'active-subaccounts':
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0">
                <h1 className="text-lg sm:text-xl font-bold text-gray-900 truncate">User Management</h1>
                <p className="text-xs sm:text-sm text-gray-500 truncate">Add users and manage sub-accounts</p>
              </div>
            </div>

            {/* Right side - Dropdown - full width on mobile, auto on desktop */}
            <div className="w-full sm:w-auto flex justify-end">
              <UserManagementDropdown selectedType={userType} onTypeChange={setUserType} />
            </div>
          </div>
        </div>
      </div>
