// src/components/common/uploader/CnicUpload.jsx
...
const CnicUploader = ({ 
  ...
  disabled = false, 
  onChange, 
  uploadText = "Upload a CNIC or drag and drop" 
}) => {
  ...
  const handleCnicChange = (e) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setError('CNIC must be less than 2MB.');
      fileInputRef.current.value = '';
      return;
    }
    ...
    reader.onloadend = () => {
      setCnicUrl(reader.result);
      if (typeof setCnicFile === 'function') {
        setCnicFile(file);
      }
