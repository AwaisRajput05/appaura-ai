...
/**
 * FilterPopup component - Desktop popup, Mobile modal
 */
export default memo(function FilterPopup({
      ...
      case 'text':
        return (
          <div key={name} className={`${isMobile ? 'flex flex-col gap-1' : 'flex items-center gap-2'}`}>
            <label className={`${isMobile ? 'w-full' : 'w-32'} text-md font-bold text-gray-700`}>
              {label}
            </label>
            <input
              ref={assignFirstRef}
              ...
              onChange={onChange}
              maxLength={11}
              className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-[#3C5690]"
            />
          </div>
        );
      ...
      case 'dateRange':
        return (
          <div key={label} className="flex flex-col gap-2">
            <label className="text-md font-bold text-gray-700">Date Range</label>
  ...
  // Desktop version (absolute positioned popup)
  if (!isMobile) {
    return (
      <div
        ref={popupRef}
